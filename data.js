/* eslint-disable */
/*
 * data.js — Lesson content only.
 *
 * Schema overview (kept generic; not language-specific):
 *
 *   LESSONS = [
 *     {
 *       id: "yapmak-etmek-tr-bn",
 *       title: "Yapmak vs Etmek",
 *       summary: "...",
 *       source: { code: "tr", label: "Turkish",  dir: "ltr", font: "tr" },
 *       target: { code: "bn", label: "Bengali",  dir: "ltr", font: "bn" },
 *       cards: [
 *         {
 *           id: "...",
 *           expression: "Yemek yapmak",   // the Turkish phrase
 *           meaning: "...",               // English gloss (1 line)
 *           verb: "yapmak",               // teaching label
 *           why: "...",                   // short language note
 *           groups: [                     // ordered semantic alignments
 *             { id: "g1", source: "...", target: "...", gloss: "..." },
 *             ...
 *           ],
 ...
 *   ]
 *
 * Important: each `groups` array is written in the natural word
 * order of the Turkish sentence, so the card reads naturally left
 * to right. Word order in Bengali will often differ; that's fine.
 * Each group carries a stable `id`, which the renderer uses to
 * assign palette slots and to cross-highlight paired phrases. So
 * meaning alignment (not positional alignment) is what matters.
 *
 * No language-specific logic lives here. For a new language pair:
 *   1. add another LESSONS entry,
 *   2. keep `groups[*]` semantically aligned (not positionally),
 *   3. the renderer handles RTL automatically because every locale
 *      block carries its own `dir`.
 *
 * Verbatim source: user-supplied table of 10 sentences (yapmak × 5,
 * etmek × 5). Card IDs match the row numbers in the source table.
 */
